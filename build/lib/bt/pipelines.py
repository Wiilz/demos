...
class BtPipeline(object):
    ...
    def __init__(self):
        self.engine = create_engine('mysql+pymysql://root:123@localhost:3306/btspider?charset=utf8mb4', echo=False)
    ...
    def process_item(self, item, spider):
        try:
            ...
            self.sess.rollback()
            print('dberror{}'.format(e.args))
            logging.DEBUG("link: {}, dberror : {}".format(item['old_link'], e.args))
        except Exception as e:
            self.sess.rollback()
            print('exception{}'.format(e.args))
            logging.ERROR("link: {}, exception : {}".format(item['old_link'], e.args))
...
if __name__ == '__main__':
    engine = create_engine('mysql+pymysql://root:123@localhost:3306/btspider?charset=utf8mb4', echo=True)  # 连接数据
